...
def transformFinalSeqs(final_seqs_input_file, final_seqs_output_file, changed_nodes):
	with open(final_seqs_input_file, 'r') as in_file:
		with open(final_seqs_output_file, 'w') as out_file:
			header = in_file.readline()
			lines = in_file.readlines()
			for i, line in enumerate(lines):
				lines[i] = [j for j in line.strip().split(',')]
				lines[i][0]= changed_nodes[int(lines[i][0])]
			out_file.write(header)
			for line in lines:
				out_file.write("{},{}\n".format(line[0], line[1]))


def adapt(node_input_file, node_output_file, edge_input_file, edge_output_file, final_seqs_input_file, final_seqs_output_file):
	...
	transformFinalSeqs(final_seqs_input_file, final_seqs_output_file, changed_nodes)
